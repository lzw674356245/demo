package laya.display {
	...
	public class Sprite extends Node implements ILayout {
		...
		/**指定当mouseThrough=true时，是否可穿透。默认值为false，如果设置为true，则点击空白区域可以穿透过去，只针对自身有效。*/
		...
		/**
		 * <p>指定鼠标碰撞检测是否优先检测自己。默认为false。</p>
		 * <p>鼠标碰撞检测是优先检测子对象，然后冒泡到父对象，如果hitTestPrior=true鼠标碰撞优先检测本对象，本对象被击中后，才进一步检测子对象。</p>
		 * <p>对于已知大小的容器（特别是根容器），设置此值为true，能减少节点碰撞，提高性能。默认为false。UI的View组建默认为true。</p>
		 */
		 ...
		 * <p>宽度默认为0，可以手动设置，或者通过getbounds获取实际宽度。设置此宽度只用来做鼠标碰撞使用，改变后并不影响显示对象大小。</p>
		 */
		 ...
		 * <p>高度默认为0，可以手动设置，或者通过getbounds获取实际宽度。设置此高度只用来做鼠标碰撞使用，改变后并不影响显示对象大小。</p>
		 */
		 ...
		 * <p>因为返回值为Sprite对象本身，所以可以使用如下语法：spr.pos(...).alpha = 0.5;</p>
		 * @param	x X 轴坐标。
		 * @param	y Y 轴坐标。
		 ...
		 * <p>因为返回值为Sprite对象本身，所以可以使用如下语法：spr.pivot(...).alpha = 0.5;</p>
		 * @param	x X轴心点。
		 * @param	y Y轴心点。
		 * @return	返回对象本身。
		 */
		 ...
		 * <p>因为返回值为Sprite对象本身，所以可以使用如下语法：spr.size(...).alpha = 0.5;</p>
		 * @param	width 宽度值。
		 * @param	hegiht 高度值。
		 * @return	返回对象本身。
		 */
		 ...
		 * <p>因为返回值为Sprite对象本身，所以可以使用如下语法：spr.scale(...).alpha = 0.5;</p>
		 * @param	scaleX X轴缩放比例。
		 * @param	scaleY Y轴缩放比例。
		 ...
		 * <p>因为返回值为Sprite对象本身，所以可以使用如下语法：spr.skew(...).alpha = 0.5;</p>
		 * @param	skewX 水平倾斜角度。
		 * @param	skewY 垂直倾斜角度。
		 * @return	返回对象本身
		 */
