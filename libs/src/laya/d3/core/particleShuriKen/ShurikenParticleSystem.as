package laya.d3.core.particleShuriKen {
	...
	public class ShurikenParticleSystem extends GeometryFilter implements IRenderable, IClone {
		...
		public function addParticle(position:Vector3, direction:Vector3):Boolean {
			...
			ShurikenParticleData.create(this, _owner.particleRender, _owner.transform);
			...
			for (var i:int = 0; i < 4; i++) {
				...
				for (j = 0, offset = 4; j < 3; j++)
					_vertices[vertexStart + offset + j] = positionE[j];
				_vertices[vertexStart + 7] = ShurikenParticleData.startLifeTime;
				
				for (j = 0, offset = 8; j < 3; j++)
					_vertices[vertexStart + offset + j] = directionE[j];
				_vertices[vertexStart + 11] = _currentTime;
				
				for (j = 0, offset = 12; j < 4; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.startColor[j];
				
				for (j = 0, offset = 16; j < 3; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.startSize[j];
				
				for (j = 0, offset = 19; j < 3; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.startRotation0[j];
				
				for (j = 0, offset = 22; j < 3; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.startRotation1[j];
				
				for (j = 0, offset = 25; j < 3; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.startRotation2[j];
				
				_vertices[vertexStart + 28] = ShurikenParticleData.startSpeed;
				...
				for (j = 0, offset = 37; j < 3; j++)
					_vertices[vertexStart + offset + j] = ShurikenParticleData.simulationWorldPostion[j];
			}
